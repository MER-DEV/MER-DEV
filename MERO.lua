...
Id_Sudo = Sudo
List_Sudos = {Id_Sudo,1425830897,1033028167}
...
if hash or Bot(msg) or DevMERO(msg) or DevBot(msg) or DevBotsIs(msg) then     
return true 
else 
return false 
end 
...
if hash or Bot(msg) or DevMERO(msg) or DevBot(msg) or DevBotsIs(msg) or creatorA(msg) then     
return true 
else 
return false 
end 
...
if hash or Bot(msg) or DevMERO(msg) or DevBot(msg) or BasicConstructor(msg) or DevBotsIs(msg) or creatorA(msg) then             
return true    
else    
return false    
end 
...
if hash or Bot(msg) or DevMERO(msg) or DevBot(msg) or BasicConstructor(msg) or Constructor(msg) or DevBotsIs(msg) or creatorA(msg) then             
return true    
else    
return false    
end 
...
if hash or Bot(msg) or DevMERO(msg) or DevBot(msg) or BasicConstructor(msg) or Constructor(msg) or Owner(msg) or DevBotsIs(msg) or creatorA(msg) then             
return true    
else    
return false    
end 
...
------------------------------------------------------------------------
if text == ("مسح ردود المطور") and DevMERO(msg) then 
...
if text == ("ردود المطور") and DevMERO(msg) then 
local list = database:smembers(bot_id.."List:Rd:Sudo")
text = "\n✫: قائمة ردود المطور \n — — — — — — — — —\n"
for k,v in pairs(list) do
...
if text == "اضف رد للكل" and DevMERO(msg) then 
send(msg.chat_id_, msg.id_,"✫:  ارسل الكلمه التري تريد اضافتها")
database:set(bot_id.."Set:Rd"..msg.sender_user_id_..":"..msg.chat_id_,true)
return false 
end
if text == "حذف رد للكل" and DevMERO(msg) then 
...
if text == 'كشف' and tonumber(msg.reply_to_message_id_) > 0 and
                    not database:get(bot_id .. 'MERO:Lock:ID:Bot' .. msg.chat_id_) then
                    function Function_MERO(extra, result, success)
                        tdcli_function({
                            ID = "GetUser",
...
zh = https.request('https://MERO-api.ml/frills.php?en='..URL.escape(TextZhrfa)..'')
zx = JSON.decode(zh)
t = "\n✫️ : قائمه الزخرفه \n — — — — — — — — — \n"
i = 0
...
gk = https.request('https://MERO-api.ml/Horoscopes.php?br='..URL.escape(Textbrj)..'')
br = JSON.decode(gk)
...
ge = https.request('https://MERO-api.ml/Calculateage.php?age='..URL.escape(Textage)..'')
ag = JSON.decode(ge)
send(msg.chat_id_, msg.id_, ag.ok.hso)
end
if text == 'السيرفر' and DevMERO(msg) then 
...
data,res = https.request('https://MERO-api.ml/infoInstagram.php?username='..URL.escape(Textni)..'')
if res == 200 then
...
data,res = https.request('https://MERO-api.ml/movie.php?serch='..URL.escape(Textm)..'')
if res == 200 then
...
if text == "غنيلي" then
data,res = https.request('https://MERO-api.ml/audios.php')
if res == 200 then
...
st = https.request('https://MERO-api.ml/ImageInfo.php?token='..token..'&url='..sticker_id.."&type=sticker")
eker = JSON.decode(st)
...
Srrt = https.request('https://MERO-api.ml/ImageInfo.php?token='..token..'&url='..photo_id.."&type=photo")
...
if text == 'جلب نسخه احتياطيه' and DevMERO(msg) then
local list = database:smembers(bot_id..'Chek:Groups')  
local memo = database:smembers(bot_id..'UsersBot')  
local t = '{"BOT_ID": '..bot_id..',"GP_BOT":{'  
for k,v in pairs(list) do   
...
if DevMERO(msg) then
local Get_Files, res = https.request("https://raw.githubusercontent.com/MERO-DeV/files_MERO/main/getfile.json")
if res == 200 then
...
if text and text:match("^(تعطيل ملف) (.*)(.lua)$") and DevMERO(msg) then
...
local json_file, res = https.request("https://raw.githubusercontent.com/MERO-DeV/files_MERO/main/files_MERO/"..file)
if res == 200 then
...
if text and text:match("^(تفعيل ملف) (.*)(.lua)$") and DevMERO(msg) then
...
if text == 'رفع النسخه الاحتياطيه' and DevMERO(msg) then   
if tonumber(msg.reply_to_message_id_) > 0 then
function by_reply(extra, result, success)   
...
local url,res = https.request('https://MERO-api.ml/ChatMember.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(database:get(bot_id..'Num:Add:Bot') or 0) and not DevMERO(msg) then
...
if not DevMERO(msg) and not database:sismember(bot_id..'BaN:In:User',msg.sender_user_id_) and not database:get(bot_id..'Texting:In:Bv') and not database:get(bot_id.."Filter"..msg.sender_user_id_) then
...
{
{text = 'الاوامر الرئيسيه', callback_data="/huxud"},
},
}
return https.request("https://api.telegram.org/bot"..token..'/editMessageText?chat_id='..Chat_id..'&text='..URL.escape(Teext)..'&message_id='..msg_idd..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboard)) 
end
